...
namespace Tanks.Complete
{
    public class GameManager : MonoBehaviour
    {
        ...
        // =========================================================
        // >> INICIO: NUEVAS VARIABLES DE ACTIVIDAD (ACTUALIZADO) <<
        // =========================================================
        private int m_DerrotasTotales = 0;           // Contador de rondas perdidas globalmente
        private const int MAX_INTENTOS_PERDIDOS = 5; // Límite de rondas perdidas (requisito 4)

        private float m_TiempoInicioPartida;         // Momento en que inicia la partida
        private float m_TiempoTotalPartida;          // Duración total de la partida (requisito 5)
        // =========================================================
        // >> FIN: NUEVAS VARIABLES DE ACTIVIDAD (ACTUALIZADO) <<
        // =========================================================
        ...
        void GameStart()
        {
            ...
            // >> INICIO: INICIALIZACIÓN DEL CRONÓMETRO (ACTUALIZADO) <<
            m_TiempoInicioPartida = Time.time;
            // >> FIN: INICIALIZACIÓN DEL CRONÓMETRO (ACTUALIZADO) <<
        ...
        private IEnumerator GameLoop()
        {
            ...
            {
                // >> INICIO: LÓGICA DE VICTORIA GLOBAL (ACTUALIZADO) <<
                m_TiempoTotalPartida = Time.time - m_TiempoInicioPartida;

                // Muestra el mensaje de victoria (con tiempo y puntaje)
                m_TitleText.text = FinalGameMessage(m_GameWinner);

                // Si el juego terminó por victorias, espera el delay final y luego reinicia la escena
                yield return m_EndWait;
                SceneManager.LoadScene(0);
                // >> FIN: LÓGICA DE VICTORIA GLOBAL (ACTUALIZADO) <<
            }
            else if (m_DerrotasTotales >= MAX_INTENTOS_PERDIDOS) // >> NUEVA CONDICIÓN DE DERROTA GLOBAL (ACTUALIZADO) <<
            {
                // Si el juego terminó por 5 derrotas, espera el delay final y luego reinicia la escena
                m_TiempoTotalPartida = Time.time - m_TiempoInicioPartida;
                m_TitleText.text = $"GAME OVER: HAS PERDIDO {MAX_INTENTOS_PERDIDOS} VECES\n\nTIEMPO TOTAL: {m_TiempoTotalPartida:F2} segundos";
                yield return m_EndWait;
                SceneManager.LoadScene(0);
            }
            else
        ...
        private IEnumerator RoundEnding()
        {
            ...
            {
                // >> INICIO: CONTADOR DE DERROTAS (ACTUALIZADO) <<
                m_DerrotasTotales++;
                // >> FIN: CONTADOR DE DERROTAS (ACTUALIZADO) <<
            }
        ...
        private string EndMessage()
        {
            ...
            // Si hubo 5 derrotas, lo notificamos para que no salga "DRAW!"
            if (m_DerrotasTotales >= MAX_INTENTOS_PERDIDOS)
                return ""; // El mensaje de derrota global se genera en GameLoop
            ...
            // Mostrar intentos restantes para dar feedback al jugador
            int intentosRestantes = MAX_INTENTOS_PERDIDOS - m_DerrotasTotales;
            if (intentosRestantes > 0)
            {
                message += $"\nINTENTOS RESTANTES: {intentosRestantes}";
            }

            // NOTA: La lógica de GameWinner ha sido movida a GameLoop, así que no se usa aquí.

            return message;
        }

        // =========================================================
        // >> INICIO: NUEVA FUNCIÓN DE MENSAJE FINAL (ACTUALIZADO) <<
        // =========================================================
        private string FinalGameMessage(TankManager winner)
        {
            // Calcula y formatea el puntaje
            int finalScore = winner.m_Wins;

            // Calcula y formatea el tiempo con 2 decimales
            string tiempoFormateado = $"{m_TiempoTotalPartida:F2}";

            // Construye el mensaje final de victoria
            string message = $"¡{winner.m_ColoredPlayerText} GANA EL JUEGO!\n\n";
            message += $"Puntuación: {finalScore} Victorias\n";
            message += $"Tiempo Total: {tiempoFormateado} segundos";

            return message;
        }
        // =========================================================
        // >> FIN: NUEVA FUNCIÓN DE MENSAJE FINAL (ACTUALIZADO) <<
        // =========================================================
